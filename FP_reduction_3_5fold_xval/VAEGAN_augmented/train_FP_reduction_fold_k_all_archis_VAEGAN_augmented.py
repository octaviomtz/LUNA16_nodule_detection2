...
#        return 1000
    ...
    def __getitem__(self,idx):
        ...
        if np.isnan(self.cand_df.iloc[idx]['coordX']): # then we're dealing with a synthetic patch and our augmentations are limited
            currPatch = np.fromfile(currFileName,dtype='int16').astype('float32')
            currPatch = currPatch.reshape((64,64,64))
            ...
            currPatch = currPatch[12:-12,12:-12,12:-12]
            
        else: # we have a real image
            currPatch = np.fromfile(currFileName,dtype='int16').astype('float32')
